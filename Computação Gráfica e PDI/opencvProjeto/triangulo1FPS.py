import cv2
import numpy as np
import time

# Função para desenhar um triângulo colorido
def draw_colorful_triangle(img, center, size, color, angle):
    height, width, _ = img.shape
    half_size = size // 2
    points = np.array([[center[0], center[1] - half_size],
                       [center[0] - half_size, center[1] + half_size],
                       [center[0] + half_size, center[1] + half_size]], np.int32)
    points = points.reshape((-1, 1, 2))
    cv2.fillPoly(img, [points], color)

    # Aplica a transformação de rotação na imagem
    M = cv2.getRotationMatrix2D(center, angle, 1)
    rotated_img = cv2.warpAffine(img, M, (width, height))

    return rotated_img

center = (50, 50)
size = 60

# Define a cor do triângulo
color = (255, 0, 0)  # Vermelho

# Loop para girar o triângulo continuamente e medir o FPS
start_time = time.time()
frame_count = 0

# Cria uma imagem preta
img = np.zeros((100, 100, 3), np.uint8)

# Desenha e rotaciona o triângulo uma vez para obter a dimensão da janela
rotated_img = draw_colorful_triangle(img, center, size, color, 0)

while True:
    # Calcula o ângulo de rotação com base no tempo
    elapsed_time = time.time() - start_time
    if elapsed_time == 0:
        fps = 0
    else:
        angle = elapsed_time * 50  # Ajuste o fator de rotação conforme necessário

        # Desenha e rotaciona o triângulo
        rotated_img = draw_colorful_triangle(img, center, size, color, angle)

        # Exibe o FPS na imagem
        fps = frame_count / elapsed_time
        cv2.putText(rotated_img, f"FPS: {fps:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)

        # Exibe a imagem na janela
        cv2.imshow('Colorful Rotating Triangle', rotated_img)

    # Atualiza o contador de frames
    frame_count += 1

    # Verifica se a tecla 'q' foi pressionada para sair do loop
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

# Fecha a janela
cv2.destroyAllWindows()
