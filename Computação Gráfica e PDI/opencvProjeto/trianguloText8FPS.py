...
# Cria uma lista de 8 imagens para cada triângulo texturizado
textured_triangles = [np.zeros((100, 100, 3), np.uint8) for _ in range(8)]
...
while True:
    ...
    if elapsed_time == 0:
        ...
        # Desenha e rotaciona os 8 triângulos texturizados
        rotated_textured_triangles = [draw_textured_triangle(img.copy(), center, size, texture, angle) for img in textured_triangles]

        # Combina as imagens horizontalmente em 2 linhas de 4
        combined_img = np.vstack(
            (np.hstack(rotated_textured_triangles[:4]), np.hstack(rotated_textured_triangles[4:]))
        )
