...
# Cria uma lista de 128 imagens para cada triângulo texturizado
textured_triangles = [np.zeros((100, 100, 3), np.uint8) for _ in range(128)]
...
while True:
    ...
    if elapsed_time == 0:
        ...
        # Desenha e rotaciona os 128 triângulos texturizados
        rotated_textured_triangles = [draw_textured_triangle(img.copy(), center, size, texture, angle) for img in textured_triangles]

        # Combina as imagens horizontalmente em 8 linhas e 16 colunas
        combined_img = np.vstack(
            [np.hstack(rotated_textured_triangles[i:i+16]) for i in range(0, 128, 16)]
        )
