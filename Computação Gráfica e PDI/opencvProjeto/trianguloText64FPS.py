...
# Cria uma lista de 64 imagens para cada triângulo texturizado
textured_triangles = [np.zeros((100, 100, 3), np.uint8) for _ in range(64)]
...
while True:
    ...
    if elapsed_time == 0:
        ...
        # Desenha e rotaciona os 64 triângulos texturizados
        rotated_textured_triangles = [draw_textured_triangle(img.copy(), center, size, texture, angle) for img in textured_triangles]

        # Combina as imagens horizontalmente em 8 linhas de 8
        combined_img = np.vstack(
            (np.hstack(rotated_textured_triangles[:8]), np.hstack(rotated_textured_triangles[8:16]), np.hstack(rotated_textured_triangles[16:24]),
             np.hstack(rotated_textured_triangles[24:32]), np.hstack(rotated_textured_triangles[32:40]), np.hstack(rotated_textured_triangles[40:48]),
             np.hstack(rotated_textured_triangles[48:56]), np.hstack(rotated_textured_triangles[56:]))
        )
