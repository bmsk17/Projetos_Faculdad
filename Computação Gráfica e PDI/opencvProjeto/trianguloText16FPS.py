...
# Cria uma lista de 16 imagens para cada triângulo texturizado
textured_triangles = [np.zeros((100, 100, 3), np.uint8) for _ in range(16)]
...
while True:
    ...
    if elapsed_time == 0:
        ...
        # Desenha e rotaciona os 16 triângulos texturizados
        ...
        combined_img = np.vstack(
            (np.hstack(rotated_textured_triangles[:4]), np.hstack(rotated_textured_triangles[4:8]), np.hstack(rotated_textured_triangles[8:12]), np.hstack(rotated_textured_triangles[12:]))
        )
