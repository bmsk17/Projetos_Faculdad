...
# Cria uma lista de 64 imagens vermelhas para cada triângulo
images = [np.zeros((100, 100, 3), np.uint8) for _ in range(64)]
...
while True:
    ...
    if elapsed_time == 0:
        ...
        # Desenha e rotaciona os 64 triângulos
        ...
        combined_img = np.vstack(
            (np.hstack(rotated_images[:8]), np.hstack(rotated_images[8:16]), np.hstack(rotated_images[16:24]),
             np.hstack(rotated_images[24:32]), np.hstack(rotated_images[32:40]), np.hstack(rotated_images[40:48]),
             np.hstack(rotated_images[48:56]), np.hstack(rotated_images[56:]))
        )
