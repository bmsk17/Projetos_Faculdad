...
void Fila<T>::ImprimeFila(){
   ...
   while (p != NULL){
    ...
    cout<< endl<<"tem algo aqui : "<<p->getItem(); //mudar
    p = p->getProx();
...
int main(){
    ...
    int n,ordemP,tamanhoP,ordem,tamanho,vert1,vert2,vInicial,vFinal,contador,doente,inicial,doentes;
    double peso,soma=0;
    Fila<int> doentess;
...
/*
    ...
    cin>>vInicial;
    cin>>vFinal;
    //cout<<"VFINAL = "<<vFinal<<" VINICIAL: "<<vInicial<<"----------------------"<<endl;
    contador = ordemP;
...
*/
    ...
    d.executaDijkstra(vInicial); //aqui quando tu montar tu passa o vertice inicial;
    Fila<int> filaa = d.getCaminhoMin(vFinal); //aqui tu passa o destino;
    ...
    while (p != NULL){
        ...
        while (don != NULL){
            ...
            if(p->getItem() == don->getItem()){
               // cout<< endl<<"WWWWWWWW : "<<p->getItem(); //mudar
               // Grafo<int> gk = neuronio[p->getItem()].getGrafo();
