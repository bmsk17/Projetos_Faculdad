...
public:
    ...
    void solucao(T antecessor,T vinicial,T vfinal,Grafo<T> g,Lista<T> *salvar);
};
...
T Grafo<T>::BFS (T iniVert, T fimVert){      //Busca em largura, que irá pegar o menor caminho entre dois vertices
    ...
    arvore.print();
    Lista<T> save;
    solucao(0,iniVert,fimVert,arvore,&save);
    save.print();

}

template<class T>
void Grafo<T>::solucao(T antecessor,T vinicial,T vfinal,Grafo<T> g,Lista<T> *salvar){
    ...
    while(posicao!=NULL){
        ...
        if(antecessor!=posicao->getItem()){
            ...
            solucao(vinicial,posicao->getItem(),vfinal,g,salvar);
        }
...
float SistemaPlanetario<T>::soma(){
    ...
    while(s != NULL){
        i = dfs.getGrafoCriado()->verificaVertice(s->getItem().getVertice1(),s->getItem().getVertice2());
        if(i==0){
