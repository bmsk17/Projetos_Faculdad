...
public:
    ...
    void solucao(T antecessor,T vinicial,T vfinal,Grafo<T> g,Lista<T> *salvar,int volta,int tamanho);
};
...
T Grafo<T>::BFS (T iniVert, T fimVert){      //Busca em largura, que irá pegar o menor caminho entre dois vertices
    ...
    arvore.print();
    Lista<T> save;
    solucao(0,iniVert,fimVert,arvore,&save,0,0);
    save.print();

}

template<class T>
void Grafo<T>::solucao(T antecessor,T vinicial,T vfinal,Grafo<T> g,Lista<T> *salvar,int volta,int tamanho){
    ...
    while(posicao!=NULL){
        ...
        if(volta==0){
            if(antecessor!=posicao->getItem()){
                ...
                salvar->insere(posicao->getItem());
                tamanhoAux++;
                solucao(vinicial,posicao->getItem(),vfinal,g,salvar,volta,tamanhoAux);
            }
        }
